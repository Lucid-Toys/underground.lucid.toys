define('./workbox-ff34be65.js', ['exports'], function (exports) {
  'use strict'
  ...
  const _isResponseSafeToCache = async ({
    ...
    if (!pluginsUsed) {
      ...
      responseToCache =
        responseToCache && responseToCache.status === 200
          ? responseToCache
          : undefined
    }
  ...
  class StaleWhileRevalidate {
    ...
    async handle({ event, request }) {
      ...
      {
        logger.groupCollapsed(
          messages$1.strategyStart('StaleWhileRevalidate', request)
        )
